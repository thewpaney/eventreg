module UserHelper
  ...
  # @brief Auto-register a user in the optimal workshop(s)
  # @param user Type User - the user to register
  # @param verbose Type Boolean - whether to print logs, defaults to true
  def auto_register(user, verbose = true)
    unless user.has_first?
      # Give them the one with the lowest fullness (amount of people/amount of spots)
      miserables = Workshop.available_for_session(user,1).sort {|w, w2| w.sfullness <=> w2.sfullness}
      force_register(user, [miserables.first.id], verbose) unless miserables == []
      puts "Unable to find First for #{user.prefix}" if verbose and miserables == []
    end
    
    unless user.has_second?
      # Give them the one with the lowest fullness (amount of people/amount of spots)
      miserables = Workshop.available_for_session(user,2).sort {|w, w2| w.sfullness <=> w2.sfullness}
      force_register(user, [miserables.first.id], verbose) unless miserables == []
      puts "Unable to find Second for #{user.prefix}" if verbose and miserables == []
    end
    
    unless user.has_third?
      # Give them the one with the lowest fullness (amount of people/amount of spots)
      miserables = Workshop.available_for_session(user,3).sort {|w, w2| w.sfullness <=> w2.sfullness}
      force_register(user, [miserables.first.id], verbose) unless miserables == []
      puts "Unable to find Third for #{user.prefix}" if verbose and miserables == []
    end
    puts user.full if verbose
  end
end

Events::App.helpers UserHelper
